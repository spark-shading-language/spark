...
More detailed reference information on language syntax and rules will be deferred to Chapter \ref{chap:langref}.
...
It also has a \emph{rate} of \code{@Fragment}, which says that this attribute will be evaluated once per fragment (you can try to read the \code{@} character as ``per-'').
...
We will discuss input attributes in more detail in Section \ref{sec:recordguide}, but in most cases these are used to mark shader parameters (\code{input @Uniform}) and outputs (\code{output @Pixel}).
...
\begin{note}
Spark does not currently expose the fixed-function rasterization state to shader writers.
This will be fixed in a future release.
At present, a Spark shader will always render using whatever rasterization state is bound to the device context.
\end{note}
