...
namespace QUT.GPGen
{
    ...
    public static class CharacterUtilities
    {
        ...
        /// <returns></returns>
        private static string GetSubstring(string input, int index, int length)
        {
        ...
        /// <returns></returns>
        private static int GetOctalChar(string source)
        {
            if (source.Length != 3)
                return -1;
            if (IsOctDigit(source[0]) && IsOctDigit(source[1]) && IsOctDigit(source[2]))
                return OctChar(source[0], source[1], source[2]);
            else
                return -1;
        }

        private static int GetHexadecimalChar(string source)
        {
            if (source.Length != 2)
                return -1;
            if (IsHexDigit(source[0]) && IsHexDigit(source[1]))
                return HexChar(source[0], source[1]);
            else
                return -1;
        }

        private static int GetUnicodeChar(string source)
        {
            uint rslt = 0;
        ...
        /// <returns>interpreted version of the string</returns>
        public static string InterpretEscapesInVerbatimString(string text)
        {
            ...
            switch (chr)
            {
                ...
                case '3':
                    ...
                    if (valu >= 0)
                        ...
                        throw new StringInterpretException(GetSubstring(source, index - 4, 4));
                ...
                case 'u': 
                    ...
                    if (valu < 0)
                        throw new StringInterpretException(GetSubstring(source, index - 6, 6));
                    //else if (valu > symLimit)
                    //    throw new StringInterpretException(GetSubstring(str, index - 6, 6));
                    else
                        return valu;
                case 'U':
                    valu = GetUnicodeChar(GetSubstring(source, index, 8)); index += 8;
                    if (valu < 0)
                        throw new StringInterpretException(GetSubstring(source, index - 10, 10));
                    //else if (valu > symLimit)
                    //    throw new StringInterpretException(GetSubstring(str, index - 10, 10));
                    else
                        return valu;
                default:
                    return (int)chr;
