...
[maxvertexcount(3)]
[instance(6)]
void GS_CubeMap_GSInst(
    triangle GS_CUBEMAP_IN input[3],
    inout TriangleStream<PS_CUBEMAP_IN> CubeMapStream,
    uint f : SV_GSInstanceID )
{
    PS_CUBEMAP_IN output;
    output.RTIndex = f;
    for( int v = 0; v < 3; v++ )
...
float4 PS_EnvMappedScene( VS_OUTPUT_SCENEENV vin ) : SV_Target
{
    ...
    float3 V = -normalize( vin.wvPos.xyz );
...
technique10 RenderCubeMap_GSInst
{
    pass p0
    {
        SetVertexShader( CompileShader( vs_4_0, VS_CubeMap() ) );
        SetGeometryShader( CompileShader( gs_5_0, GS_CubeMap_GSInst() ) );
        SetPixelShader( CompileShader( ps_4_0, PS_CubeMap() ) );
