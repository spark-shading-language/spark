...
// SPARK:
#include "PNTriangles.spark.h"
bool gUseSpark = true;
spark::IContext* gSparkContext = nullptr;
spark::IModule* gSparkModule = nullptr;
Base* gSparkShaderInstance = nullptr;
...
// SPARK:
#define IDC_CHECKBOX_USE_SPARK                  33
...
// SPARK:
void FinalizeApp();
Base* CreateSparkShaderInstance();
...
void RenderMeshSpark( CDXUTSDKMesh* pDXUTMesh, UINT uMesh, 
                 D3D11_PRIMITIVE_TOPOLOGY PrimType = D3D11_PRIMITIVE_TOPOLOGY_UNDEFINED, 
                 UINT uDiffuseSlot = INVALID_SAMPLER_SLOT, UINT uNormalSlot = INVALID_SAMPLER_SLOT,
                 UINT uSpecularSlot = INVALID_SAMPLER_SLOT );
...
void InitApp()
{
    ...
    // SPARK:
    g_HUD.AddCheckBox( IDC_CHECKBOX_USE_SPARK, L"Use Spar(K)", 0, iY += 25, 140, 24, gUseSpark, 'K' );
    ...
    WCHAR str[MAX_PATH];
    DXUTFindDXSDKMediaFileCch( str, MAX_PATH, L"PNTriangles.spark" );
    char sparkShaderFilePath[MAX_PATH];
    int len = WideCharToMultiByte(CP_ACP, 0, str, wcslen(str), sparkShaderFilePath, MAX_PATH, nullptr, nullptr);
    sparkShaderFilePath[len] = 0;

    gSparkModule = gSparkContext->CompileFile( sparkShaderFilePath );
}
...
void CALLBACK OnGUIEvent( UINT nEvent, int nControlID, CDXUTControl* pControl, void* pUserContext )
{
    ...
    switch( nControlID )
    {
        ...
        case IDC_CHECKBOX_TEXTURED:
            SAFE_RELEASE( gSparkShaderInstance );
            break;
...
inline spark::float4 Convert( D3DXVECTOR3 v )
{
    spark::float4 result;
    result.x = v.x;
    result.y = v.y;
                                 ...
                                 float fElapsedTime, void* pUserContext )
{
    ...
    {
        if( gSparkShaderInstance == nullptr )
        {
            gSparkShaderInstance = CreateSparkShaderInstance();
        }
        ...
        gSparkShaderInstance->SetLightDir( Convert( v3LightDir ) );
        gSparkShaderInstance->SetEye( Convert( *g_Camera[g_eMeshType].GetEyePt() ) );
        gSparkShaderInstance->SetViewVector( Convert( v3ViewVector ) );

        gSparkShaderInstance->SetTessFactors( spark::float4(
            (float)g_uTessFactor,
            (float)g_uTessFactor,
            (float)g_v3AdaptiveTessParams[g_eMeshType].x,
            (float)g_v3AdaptiveTessParams[g_eMeshType].y ) );

        gSparkShaderInstance->SetScreenParams( spark::float4(
            (float)DXUTGetDXGIBackBufferSurfaceDesc()->Width,
            (float)DXUTGetDXGIBackBufferSurfaceDesc()->Height,
            0.0f, 0.0f ) );

        gSparkShaderInstance->SetGuiParams1( spark::float4(
            g_fBackFaceCullEpsilon,
            ( g_fSilhoutteEpsilon > 0.99f ) ? ( 0.99f ) : ( g_fSilhoutteEpsilon ),
            g_fRangeScale,
            (float)g_uEdgeSize ) );

        gSparkShaderInstance->SetGuiParams2( spark::float4(
            g_fResolutionScale,
            ( ( g_fViewFrustumCullEpsilon * 2.0f ) - 1.0f ) * g_v3AdaptiveTessParams[g_eMeshType].z,
            0.0f, 0.0f ) );

        gSparkShaderInstance->SetViewFrustumPlanes(
            spark::Array<spark::float4, 4>(reinterpret_cast<spark::float4*>(f4ViewFrustumPlanes)) );

        gSparkShaderInstance->SetSamplePoint( g_pSamplePoint );
        gSparkShaderInstance->SetSampleLinear( g_pSampleLinear );
        gSparkShaderInstance->SetTxDiffuse( g_pDiffuseTextureSRV );

        // \todo: set rasterizer state
        ...
        for( int iMesh = 0; iMesh < (int)g_SceneMesh[g_eMeshType].GetNumMeshes(); iMesh++ )
        {
            RenderMeshSpark( &g_SceneMesh[g_eMeshType], (UINT)iMesh, PrimitiveTopology, uDiffuseSlot );
        }
    }
...
// SPARK:
//--------------------------------------------------------------------------------------
// Create a Spark shader for a particular combination of effects
//--------------------------------------------------------------------------------------
Base* CreateSparkShaderInstance()
{
    static const int MAX_SPARK_MIXINS = 10;
    int mixinCount = 0;
    spark::IShaderClass* mixins[MAX_SPARK_MIXINS];

    mixins[mixinCount++] = gSparkModule->FindShaderClass<Base>();

    // Tessellation
    if( g_SampleUI.GetCheckBox( IDC_CHECKBOX_TESSELLATION )->GetChecked() )
    {
        mixins[mixinCount++] = gSparkModule->FindShaderClass<PNTriangles>();
    }

    // Texturing
    if( g_SampleUI.GetCheckBox( IDC_CHECKBOX_TEXTURED )->GetChecked() && g_SampleUI.GetCheckBox( IDC_CHECKBOX_TEXTURED )->GetEnabled() )
    {
        mixins[mixinCount++] = gSparkModule->FindShaderClass<Texturing>();
    }

    // Back face culling
    if( g_SampleUI.GetCheckBox( IDC_CHECKBOX_BACK_FACE_CULL )->GetChecked() )
    {
        mixins[mixinCount++] = gSparkModule->FindShaderClass<BackFaceCulling>();
    }

    // View frustum culling
    if( g_SampleUI.GetCheckBox( IDC_CHECKBOX_VIEW_FRUSTUM_CULL )->GetChecked() )
    {
        mixins[mixinCount++] = gSparkModule->FindShaderClass<ViewFrustumCulling>();
    }

    // Screen space adaptive tessellation
    if( g_SampleUI.GetCheckBox( IDC_CHECKBOX_SCREEN_SPACE_ADAPTIVE )->GetChecked() )
    {
        mixins[mixinCount++] = gSparkModule->FindShaderClass<ScreenSpaceAdaptiveTessellation>();
    }

    // Distance adaptive tessellation (with screen resolution scaling)
    if( g_SampleUI.GetCheckBox( IDC_CHECKBOX_DISTANCE_ADAPTIVE )->GetChecked() )
    {
        mixins[mixinCount++] = gSparkModule->FindShaderClass<DistanceAdaptiveTessellation>();
    }

    // Screen resolution adaptive tessellation
    if( g_SampleUI.GetCheckBox( IDC_CHECKBOX_SCREEN_RESOLUTION_ADAPTIVE )->GetChecked() )
    {
        mixins[mixinCount++] = gSparkModule->FindShaderClass<ScreenResolutionAdaptiveTessellation>();
    }

    // Orientation adaptive tessellation
    if( g_SampleUI.GetCheckBox( IDC_CHECKBOX_ORIENTATION_ADAPTIVE )->GetChecked() )
    {
        mixins[mixinCount++] = gSparkModule->FindShaderClass<OrientationAdaptiveTessellation>();
    }

    spark::IShaderClass* composed = gSparkModule->CreateShaderClass(
        mixinCount,
        mixins );
    ...
    UINT uDiffuseSlot, UINT uNormalSlot, UINT uSpecularSlot )
{
    ...
    gSparkShaderInstance->SetVertexStream( spark::d3d11::VertexStream(
        pDXUTMesh->GetVB11( uMesh, 0 ),             // buffer
        0,                                          // offset
        pDXUTMesh->GetVertexStride( uMesh, 0 ) ) ); // stride

    ID3D11Buffer* pIB = pDXUTMesh->GetIB11( uMesh );
    DXGI_FORMAT ibFormat = pDXUTMesh->GetIBFormat11( uMesh );
    ...
    for( UINT uSubset = 0; uSubset < pMesh->NumSubsets; uSubset++ )
    {
        ...
        if( uDiffuseSlot != INVALID_SAMPLER_SLOT && !IsErrorResource( pMat->pDiffuseRV11 ) )
        {
            gSparkShaderInstance->SetTxDiffuse( pMat->pDiffuseRV11 );
        }

        spark::d3d11::DrawSpan drawSpan = spark::d3d11::DrawIndexed(
            pIB,
            ibFormat,
        ...
        drawSpan.primitiveSpan.primitiveTopology = PrimType;

        gSparkShaderInstance->SetDrawSpan( drawSpan );

        gSparkShaderInstance->Submit(
            DXUTGetD3D11Device(),
...
HRESULT CreateHullShader()
{
    ...
    // SPARK:
    // Whenever we need to recompile the HLSL hull shader,
    // we will also need to create new composition of Spark
    // shader classes.
    SAFE_RELEASE( gSparkShaderInstance );
